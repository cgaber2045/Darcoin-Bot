...
function removeItem(user, item) {
  ...
  var idx = items.findIndex(p => p==item);
...
function items(interaction, args) {
  ...
  if (products.has(product)) {
      if (getWallet(interaction.user.id) >= products.get(product)) {
          subtractPoints(interaction.user.id, products.get(product));
          addPoints(defaultConfig.botId, products.get(product));
...
function getProducts() {
  var productList = [];
  for(const [key, description, value] of products) if (key != undefined) productList.push({"name": `${key} (${value} ${appConfig.coinName}) - ${description}`, "value": key});
  return productList;
}
