...
// Getters and setters
function payPoints(sender, recipient, amount) {
...
function payUser(message, args) {
  ...
  if (sender == recipient) return message.reply("Can't send money to yourself!");
  if (!(allCoinWallets.has(sender) && allCoinWallets.has(recipient))) return message.reply("Both of you must have a coin wallet to send money!");
  if (!Number.isInteger(amount)) return message.reply("You inputed something that was not a whole number.");
  if (!(allCoinWallets.get(sender) - amount >= 0 && amount > 0)) return message.reply("You broke boy what are you doing trying to send money.");
...
function items(interaction, args) {
  ...
  if (products.has(product)) {
      if (getWallet(interaction.user.id) >= products.get(product)[1]) {
          ...
          payPoints(interaction.user.id, defaultConfig.botId, products.get(product)[1]);
...
module.exports = {commands, addPoints, subtractPoints, payPoints, setWallet, getWallet, balance, 
