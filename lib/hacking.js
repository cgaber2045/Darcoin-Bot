...
function hackingGame(interaction, args) {
  ...
  if (wallet.hasItem(sender, "dongle")) {
    wallet.removeItem(sender, "dongle");
  } else return interaction.reply(`You need a dongle in order to hack!`);
  ...
  if (hackCooldowns.has(sender) && hackCooldowns.get(sender) >= Date.now()) {
    ...
    return message.reply("Relax buddy, you just tried to rob someone! You are on a robbing cooldown until " + 
  ...
  if (!(wallet.has(sender) && wallet.has(recipient))) return message.reply("Both of you must have a coin wallet to attempt to rob money!");

  // Checking amount input
  if (!Number.isInteger(amount)) return message.reply("You inputed something that was not a whole number.");

  // Checking that they have the entry fee
  if (!(wallet.getWallet(sender) - (Math.round(gameObject.amount * appConfig.penaltyAmount)) >= 0)) 
    return message.reply("You must have at least " + appConfig.penaltyAmount * 100 + "% of the amount you are trying to rob in case you fail.");

  // Checking that they are only trying to hack what they have
  if (!(wallet.getWallet(recipient) - gameObject.amount >= 0 && gameObject.amount > 0)) return message.reply("You can't take more than what they have.");
