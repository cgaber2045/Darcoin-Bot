...
function hackingGame(interaction, args) {
  if (!interaction.channel)
    return interaction.reply(`Hacking currently not supported in DM's!`);

  var date = new Date();
  if ( !(date.getDay() === 6 || date.getDay() === 3) ) 
    return interaction.reply(`Hacking is only available on Wednesday and Saturday!`);

  var sender = interaction.user.id;
  var recipient = defaultConfig.botId; //args.get('user').value;
  ...
  var difficultyRating = 8;

  var width, height;
  height = ((recipient == defaultConfig.botId) ? 8 : 8) + difficultyRating;
  width = 10;

  var startPos = {x: crypto.randomInt(0, width), y: crypto.randomInt(0, Math.round(height/4))};
  var endPos = {x: crypto.randomInt(0, width), y: crypto.randomInt(Math.round(3*height/4), height)};
  ...
  if (hackCooldowns.has(sender) && hackCooldowns.get(sender) >= Date.now()) {
    ...
    return interaction.reply("Relax buddy, you just tried to rob someone! You are on a robbing cooldown until " + 
    d.toLocaleString('en-US', { month:'numeric', day:'numeric', hour:'numeric', minute: 'numeric', hour12: true}) + " EST.");
  }
  // Checking if both parties have wallets
  if (!(wallet.has(sender) && wallet.has(recipient))) return interaction.reply("Both of you must have a coin wallet to attempt to rob money!");

  // Checking amount input
  if (!Number.isInteger(gameObject.amount)) return interaction.reply("You inputed something that was not a whole number.");

  // Checking that they have the entry fee
  if (!(wallet.getWallet(sender) - (Math.round(gameObject.amount * appConfig.penaltyAmount)) >= 0)) 
    return interaction.reply("You must have at least " + appConfig.penaltyAmount * 100 + "% of the amount you are trying to rob in case you fail.");

  // Checking that they are only trying to hack what they have
  if (!(wallet.getWallet(recipient) - gameObject.amount >= 0 && gameObject.amount > 0)) return interaction.reply("You can't take more than what they have.");

  if (!wallet.hasItem(sender, "dongle")) return interaction.reply(`You need a dongle in order to hack!`);

  wallet.removeItem(sender, "dongle");
...
function gameDisplay(interaction, gameObject) {
  ...
  var sender = interaction.user.id;
  var recipient = defaultConfig.botId;//interaction.options.get('user').value;
  ...
  if (!interaction.replied) {
    ...
    if (!gameObject.finished && gameObject.currentPos.x == gameObject.end.x && gameObject.currentPos.y == gameObject.end.y) {
      ...
      wallet.payPoints(recipient, sender, amount);
      ...
      wallet.payPoints(sender, recipient, amount);
      gameObject.finished = true;
