...
function hackingGame(interaction, args) {
    ...
    var width, height;
    width = ((recipient == defaultConfig.botId) ? 12 : 6) + difficultyRating;
    height = 10;
...
function gameDisplay(interaction, gameObject) {
  ...
  if (!interaction.replied) {
    ...
    if (gameObject.currentPos.x == gameObject.end.x && gameObject.currentPos.y == gameObject.end.y) {
      ...
      displayString += `\'${wallet.getNickFromID(sender)} has lost ${Math.round(gameObject.amount * appConfig.penaltyAmount)} ${appConfig.coinName}!\'\n`;
