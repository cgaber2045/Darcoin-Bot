...
function hackingGame(interaction, args) {
    ...
    var width, height;
    width = recipient == defaultConfig.botId ? 20 : 10;
    height = recipient == defaultConfig.botId ? 10 : 7;

    var startPos = {x: crypto.randomInt(0, width/2), y: crypto.randomInt(0, height)};
    var endPos = {x: crypto.randomInt(width/2, width), y: crypto.randomInt(0, height)};
  ...
  for (let row = 0; row < board.length; row++) {
    ...
    for (let cell = 0; cell < board[row].length; cell++) {
      ...
      rowString += board[row][cell].right ? wallChar : 
        ...
        && ( gameObject.path.some(pos => ((pos.x == cell+1 || pos.x == cell-1) && pos.y == row))) ? pathChar : spaceChar; 

      // If we can move down it is a space otherwise it is a wall.
      nextRowString += board[row][cell].bottom ? wallChar : 
        gameObject.path.some(pos => (pos.x == cell && pos.y == row))
        && ( gameObject.path.some(pos => (pos.x == cell && (pos.y == row+1 || pos.y == row-1)))) ? pathChar : spaceChar; 
      nextRowString += wallChar;
    }
    rowString += '\n' + nextRowString;
...
function gameDisplay(interaction, gameObject) {
  ...
  if (!interaction.replied) {
    ...
    if (gameObject.currentPos.x == gameObject.end.x && gameObject.currentPos.y == gameObject.end.y) {
      displayString += `\'${wallet.getNickFromID(sender)} has successfully completed the hack!\'\n`;
      displayString += `\'${wallet.getNickFromID(sender)} has earned ${gameObject.amount} ${appConfig.coinName}!\'\n`;
      wallet.addPoints(sender, gameObject.amount);
      wallet.subtractPoints(recipient, gameObject.amount);
      wallet.updateMessage();
    } else if (gameObject.timelimit <= 0) {
      displayString += `\'${wallet.getNickFromID(sender)} has failed the hack!\'\n`;
      displayString += `\'${wallet.getNickFromID(sender)} has lost ${gameObject.amount} ${appConfig.coinName}!\'\n`;
      wallet.subtractPoints(sender, Math.round(gameObject.amount * appConfig.penaltyAmount));
      wallet.addPoints(recipient, Math.round(gameObject.amount * appConfig.penaltyAmount));
      wallet.updateMessage();
