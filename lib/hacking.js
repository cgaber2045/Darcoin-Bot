...
function hackingGame(interaction, args) {
  ...
  if (wallet.hasItem(sender, "dongle")) {
    ...
    interaction.reply(`You need a dongle in order to hack!`);
    return;
  }
  ...
  if (!hackCooldowns.has(sender) || !(hackCooldowns.get(sender) >= Date.now())) {
      ...
      if (wallet.has(sender) && wallet.has(recipient)) {
          ...
          if (Number.isInteger(gameObject.amount)) {
              ...
              if (wallet.getWallet(sender) - (Math.round(gameObject.amount * appConfig.penaltyAmount)) >= 0) {
                  ...
                  if (wallet.getWallet(recipient) - gameObject.amount >= 0 && gameObject.amount > 0) {
