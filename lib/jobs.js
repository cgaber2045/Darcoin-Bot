...
function createRoles() {
  ...
  joblist.forEach((amt, role) => {
    if (!guild.roles.find(x => x.name === role)) {
...
function dailyIncome() {
  ...
  allwallets.forEach((amt, userId) => {
    ...
    var userRoles = guild.members.get(userId).roles;
    userRoles.forEach((role) => {
      if (joblist.get(role.name)) {
        ...
        bot.channels.get(defaultConfig.generalChannelID).send(`${wallet.getNickFromID(userId)} has recieved ${joblist.get(role.name)} ${appConfig.coinName} for being a ${role.name}!\n`)
...
function hire(message, user, role) {
  if (wallet.has(user)) {
    if (joblist.get(role.charAt(0).toUpperCase() + role.slice(1))) {
      ...
      var roleId = message.guild.roles.find(x => x.name.toLowerCase() === role.toLowerCase()).id;
