...
function play(guild, song) {
  ...
  const dispatcher = serverQueue.connection.playStream(
    ytdl(song.url, {filter : 'audioonly', dlChunkSize: 0, highWaterMark: 1<<25})
  )
  dispatcher.setVolumeLogarithmic(serverQueue.volume / 5);
  
  // Different event handlers used for debugging!
  dispatcher.on('start', () => {console.log("Dispatcher Starting!");});
  dispatcher.on('error', error => console.log("Dispatcher error: " + error));
  serverQueue.connection.on('error', error => console.log("Voice Connection Error: " + error));
  serverQueue.connection.on('failed', error => console.log("Voice Connection Failure: " + error));
  serverQueue.connection.on('debug', message => console.log("Voice Connection Debug: " + message));
