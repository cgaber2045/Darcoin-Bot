...
async function execute(interaction, args) {
  ...
  if (!serverQueue) {
    ...
    if (wallet.getWallet(interaction.user.id) < song.cost)
      return interaction.reply(`You don't have enough money to play a song!`);
  ...
  // If we already have a server queue
    ...
    // makes sure you have money to play the song
    if (wallet.getWallet(interaction.user.id) < song.cost)
      return interaction.reply(`You dont have enough money to play a song.`);
...
function play(guild, song) {
  ...
  serverQueue.audioPlayer.on(AudioPlayerStatus.Playing, () => {
    if (song.playing) return; // Fixes bug where song is playing twice.
    song.playing = true;

    console.log(`${song.title} Started Playing!`);

    // Charges user for the song
    wallet.payPoints(song.requester, defaultConfig.botId, song.cost);
    ...
    // Create embed
    sendMessage(serverQueue, serverQueue.audioPlayer, song);
...
function skip(interaction) {
  ...
  if (interaction.user.id != serverQueue.songs[0].requester && wallet.getWallet(interaction.user.id) <= appConfig.skipCost)
    return interaction.reply(`You cannot afford to skip the song!`);
  ...
  if (interaction.user.id != serverQueue.songs[0].requester) {
    ...
    wallet.payPoints(interaction.user.id, defaultConfig.botId, appConfig.skipCost);
...
function sendMessage(serverQueue, player, song) {
  ...
  let footer = `Time Remaining: ${new Date((song.length - seek) * 1000).toISOString().substr(14, 5)}`;
  if (song.length > 0) nowPlaying.setFooter({text: footer});
