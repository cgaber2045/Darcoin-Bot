...
function play(guild, song) {
  ...
  const dispatcher = serverQueue.connection.play(
    ytdl(song.url, {filter : 'audioonly', dlChunkSize: 0, highWaterMark: 1<<25, volume: false, requestOptions: {headers: {Cookie: cookieString}}}), {bitrate: 96, volume: false} 
  )
...
function sendMessage(serverQueue, dispatcher, song) {
  var seek = Math.ceil(dispatcher.streamTime / 1000);
