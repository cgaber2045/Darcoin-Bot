...
function userHit(cardsObj, message) {
    ...
    if (total > 21) {
        ...
        try {messageMap.delete(message.author.id)} catch(error){ console.log(error) };
    }
    else {
        ...
        collector.on('collect', m => {
            ...
            try {m.delete()} catch(error){ console.log(error) };
        });
...
function botHit(cardsObj, message) {
    ...
    if (botTotal > 21) {
        ...
        if (botTotal == 21 && cardsObj.botTotal.length == 2) {
            // Computer has blackjack - end game (user loss)
            wonGames++;
            console.log("Winrate: " + (wonGames / (wonGames + lostGames)));
            ...
            wallet.subtractPoints(defaultConfig.botId, bet);
            wallet.updateMessage();
            var status = "\"" + appConfig.coinName + ` stands with ${botTotal}: **${wallet.getNickFromID(user.id)} Wins**\"`;
            gameDisplay(message, cardsObj, status);
            try { messageMap.delete(message.author.id) } catch(error){ console.log(error) };
        }
    } 
...
function startGame(message, args) {
    ...
    if (( bet >= appConfig.minBet && bet <= appConfig.maxBet) || (jobs.hasJob(message.author.id) && bet >= appConfig.minBet && bet <= appConfig.maxBet * 2) ) {
        if (bet && !isNaN(bet) && (bet <= bal)) {
            ...
            } else message.channel.send(`The bot doesn't have enough to bet!`);
        } else message.channel.send(`Please specify a valid bet that is less than or equal to your current balance.\nEx: \`!blackjack 50\` starts a game with 50 ${appConfig.coinName} at stake.`);
    } else message.channel.send(`The max bet for on normal tables is ${appConfig.maxBet}, highroller max bet is ${appConfig.maxBet * 2}, and the minimum bet is ${appConfig.minBet}!`);
}
...
function gameDisplay(message, cardsObj, status) {
    ...
    var gamemsg = `Welcome To ${appConfig.coinName} ${jobs.hasJob(message.author.id) ? '\'HIGHROLLER\' ' : ''}Blackjack! Get A Higher Total To Earn Your Bet, Or A Total Of 21 For 3:2 Your Bet. \n
