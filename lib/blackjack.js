...
function userHit(cardsObj, message) {
    ...
    if (total > 21) {
        ...
        var status = `\"${wallet.getNickFromID(user)}, your total is ${total}, hit or stand? Type 'hit' or 'stand'.\"`;
        gameDisplay(message, cardsObj, status);

        const filter = m => ( m.content.toLowerCase() === 'hit' || m.content.toLowerCase() === 'stand') && m.author.id === user;
    ...
    if (( bet >= appConfig.minBet && bet <= appConfig.maxBet) || (jobs.hasJob(user) && bet >= appConfig.minBet && bet <= appConfig.maxBet * 2) ) {
        if (bet && !isNaN(bet) && (bet <= bal)) {
            if (bet * 2 <= botbal) {
                ...
                gameDisplay(message, cardsObj, `\"${wallet.getNickFromID(user)}, hit or stand? Type 'hit' or 'stand'.\"`);
