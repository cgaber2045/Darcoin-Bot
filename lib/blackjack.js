...
function userHit(cardsObj, message) {
    ...
    if (total > 21) {
        ...
        const collector = message.channel.createMessageCollector(filter, {maxMatches: 1, time: 100000});

        collector.on('collect', m => {
            if (m.content.toLowerCase() === 'hit') userHit(cardsObj, message);
            else if (m.content.toLowerCase() === 'stand') userStand(cardsObj, message);
            message.channel.fetchMessage(m).then(msg => msg.delete().catch(() => console.log("Can't delete dm message!")));
        });
...
function startGame(message, bet) {
    var user = message.author;
