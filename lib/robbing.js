...
function rob(message, args) {
    ...
    if (!(wallet.getWallet(recipient) - amount >= 0 && amount > 0)) return message.reply("You can't take more than what they have.");
    ...
    if (success) {
        ...
        message.reply("You have sucessfully robbed <@" + recipient + "> for " + amount + " " + appConfig.coinName +"! (Odds: " + (successrate * 100).toFixed(3) + "%)");
    } else {
        // We have failed the rob attempt at the person.
        wallet.payPoints(sender, recipient, Math.round(amount * appConfig.penaltyAmount));
        message.reply("You have failed to rob " + amount + " " + appConfig.coinName + " from " + wallet.getNickFromID(recipient) + ". You lost " + Math.round(amount * appConfig.penaltyAmount) + " " + appConfig.coinName +"! (Odds: " + (successrate * 100).toFixed(3) + "%)");
    }
}
