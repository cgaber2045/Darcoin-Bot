...
function bet(interaction, args) {
  ...
  if (!(bet && bet.predictionsEnabled)) return interaction.reply("The bettings are closed!");
  if (!wallet.has(sender)) return interaction.reply("You must have a coin wallet to bet money!");
  if (!Number.isInteger(amount)) return interaction.reply("You inputed something that was not a whole number.");
  if (bet.predictionAmounts.has(sender)) return interaction.reply("You have already bet!");
  if (!(bet.objectOne.toLowerCase() === object.toLowerCase() || bet.objectTwo.toLowerCase() === object.toLowerCase()))
    return interaction.reply("The options for the bet are " + bet.objectOne + " and " + bet.objectTwo);
...
function winner(interaction, args) {
  ...
  if (bet.predictionAmounts.size == 0) return interaction.reply("No people entered in betting!");
  if (bet.predictionsEnabled) return interaction.reply("You must end predictions to declare a winner!");
  if (!(winner.toLowerCase() === bet.objectOne.toLowerCase() || winner.toLowerCase() === bet.objectTwo.toLowerCase())) 
    return interaction.reply("Winner is not one of the objects! Please enter " + bet.objectOne + " or " + bet.objectTwo + ".");
