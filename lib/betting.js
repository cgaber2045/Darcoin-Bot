...
function bet(message, args) {
    ...
    if (wallet.has(sender)) {
      if (Number.isInteger(amount)) {
        if (!predictionAmounts.has(sender)) {
          ...
          updateBets();
          } else message.reply("The options for the bet are " + firstBettingThing + " and " + secondBettingThing);
  ...
  } else message.reply("The betting is closed!");
}
...
function startBetting(message, args) {
  ...
  if (!predictionsEnabled || predictionAmounts.size > 0) {
    ...
    firstBettingThing = args[0];
    secondBettingThing = args[1];

    let channel = bot.channels.cache.get(defaultConfig.generalChannelID);

    var betmessage = " ".repeat(appConfig.progressBarLength * 1) + "**Betting has started! Who will win?!**\n";
    betmessage += "**" + firstBettingThing + "** " + createBar.filledBar(appConfig.progressBarLength, 0, appConfig.progressBarLength)[0] + " **" + secondBettingThing +"**\n";
    ...
    betmessage += " ".repeat(appConfig.progressBarLength * 1) + "Total bet amount: " + (total1 + total2) + " " + appConfig.coinName + "!"; 

    message.reply("Betting has started!");
    channel.send(betmessage).then(sent => { betMessageId = sent.id } );
  } else message.reply("There is another bet in progress!");
}

// Function used to close betting
function stopBetting(message) {
  if (message.author.id != getBetStarter()) return;
  ...
  if (predictionAmounts.size > 0) {
      if (!predictionsEnabled) {
        if (winner.toLowerCase() === firstBettingThing.toLowerCase() || winner.toLowerCase() === secondBettingThing.toLowerCase()) {
          var percentOfBets = 0;
          if (winner.toLowerCase() === firstBettingThing.toLowerCase()) percentOfBets = total1 / (total1+total2);
          else percentOfBets = total2 / (total1+total2);
        ...
        } else message.reply("Winner is not one of the things! Please enter " + firstBettingThing + " or " + secondBettingThing + ".");
...
function cancelBets(message) {
  ...
  if (!isAdmin || message.author.id != getBetStarter()) return;
  let channel = bot.channels.cache.get(defaultConfig.generalChannelID);
...
function updateBets() {
  ...
  channel.messages.fetch(betMessageId).then(
    message=>{
