...
function timeout(interaction, args) {
  ...
  var time = parseInt(args.get('optional').value);
  if (!time) time = 5;
  if (userMember != null) {
...
function taze(interaction, args) {
  ...
  if (userMember != null) {
    if (userMember.voice.channel) {
      if (userMember.bannable) {
        try {
          interaction.userMember.voice.kick();
...
function buy(interaction, args) {
  ...
  if (args.data.length > 0)
    if (products.has(product)) {
      if (wallet.getWallet(interaction.user.id) >= products.get(product)) {
        if (eval(product)(interaction, args)) {
          if (product == "timeout") {
            var time = parseInt(args.get('optional').value);
            if (!time) time = 5;
            wallet.subtractPoints(interaction.user.id, products.get(product) * time);
            wallet.addPoints(defaultConfig.botId, products.get(product) * time);
            interaction.reply("You have purchased " + product + " for " + products.get(product) * time + " " + appConfig.coinName + "!");
          } else {
            wallet.subtractPoints(interaction.user.id, products.get(product));
    ...
    else interaction.reply("Wrong arguments - !buy <product> <name of person> <other...> - Use this command to purchase different products from the marketplace!");
}

function getRoles(interaction) {
  if (roleMap.has(interaction.user.id)) {
    var guild = bot.guilds.cache.get(defaultConfig.serverID);
    guild.member(interaction.user.id).roles.add(roleMap.get(interaction.user.id));
    guild.member(interaction.user.id).setNickname(nickMap.get(interaction.user.id));
