...
function shoot(message, args) {
  ...
  if (guildMember != null) {
    if (guildMember.bannable) {
      ...
      guild.member(user).send("You have been banned by " + wallet.getNickFromID(message.author.id) + ". Type !getroles when you get back.").then(
        ()=>{guild.member(user).ban("Someone paid to have you banned. LOL.")}
      );
...
function revive(message, args) {
  ...
  if (guild.member(user) != null) {
    guild.member(user).unban("Someone paid to have you unbanned... Pogchamp!").then(() => {
    ...
    if (guildMember.bannable) {
      ...
      message.channel.createInvite()
        ...
        .then(
          ()=>{
              setTimeout(() => {
                guildMember.kick("Someone paid to have you kicked. LOL.")
              }, 200);
            }
        );
      return true;
